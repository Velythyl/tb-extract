        ...
        try:
            ...
            for smoothing_id, smoothing in enumerate(SMOOTHINGS):
                ...
                if False:# DEBUG:
                    ...
                    do_finish_plot(fig=fig, axes=axes, csv_name=csv, exp_name=path, smoothing_id=smoothing)
                    plt.show()
                    exit()
...
def init_plot(solo):
    if solo:
        fig, axes = plt.subplots(1, 1, figsize=(6, 6), sharey=True, squeeze=False)
    else:
        fig, axes = plt.subplots(2, 4, figsize=(6*2, 6*4), sharey=True, squeeze=False)
    return fig, axes

def do_finish_plot(fig, axes, csv_name, exp_name, smoothing_id):
    plt.xlabel("Timestep")
...
def just_one_plot(axes, data, exp_name, subplot_x, subplot_y):
    ...
    fake_ax = sns.lineplot(axes=ax, x=x_values, y=y_values, label=line_label)
#    plt.close(fake_ax.fig)
        ...
        try:
            for smoothing_id in range(len(SMOOTHINGS)):
                ...
                for dict_id, dict in enumerate(data):
                    packing = dict[key]
                    just_one_plot(data=packing[smoothing_id], exp_name=names[dict_id], axes=axes, subplot_y=0, subplot_x=0)
                do_finish_plot(fig=fig, csv_name=key, exp_name=names[0], smoothing_id=smoothing_id, axes=axes)
            #plt.show()
            #exit()
        ...
        for key in all_common_keys:
            ...
            for index in indices:
                for dict_id, dict in enumerate(results[index]):
                    packing = dict[dict_id][key]
                    just_one_plot(axes=axes, data=packing[smoothing_id], exp_name=exp_pairs[index][dict_id], subplot_x=index % 2, subplot_y=index % 4) # todo
            do_finish_plot(fig=fig, csv_name=key, exp_name=exp_pairs[index][0], smoothing_id=smoothing_id)
            #plt.show()
            #exit()
